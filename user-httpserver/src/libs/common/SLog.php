...
class SLog {
    ...
    public static function config()
    {
        if (!isset( self::$config) ) {
            self::$config = \Phalcon\DI::getDefault()->getShared('config');
        }
    }
    ...
    public static function writeLog($msg, $errorType = self::INFO, $filePath=''  ) {
        ...
        try {
            ...
            $formatter->jsonData['logTraceId']   = $session->getId();
            $formatter->jsonData['logSessionId'] = $session->getId();
            $formatter->jsonData['logClientIp']  = Common::getClientIp();
            $formatter->jsonData['logDate']      = date('Y-m-d H:i:s', time());
            $formatter->jsonData['logAppId']     = PROJECT_NAME;
            if( PHP_SAPI  == "fpm-fcgi"){
                $formatter->jsonData['logUrl']        = \Phalcon\DI::getDefault()->getShared('dispatcher')->getControllerName() . "-" . \Phalcon\DI::getDefault()->getShared('dispatcher')->getActionName();
            }
