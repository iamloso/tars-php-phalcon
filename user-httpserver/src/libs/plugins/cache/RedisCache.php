...
class RedisCache
{
    ...
    function __construct ($DbConfig, $redisName='redis')
    {
        $this->redis = new \Redis();

        $this->redis->connect($DbConfig->$redisName->host, $DbConfig->$redisName->port);
    

        if(!empty($DbConfig->$redisName->auth)){
            $this->redis->auth($DbConfig->$redisName->auth);
        }
        $this->redis->select($DbConfig->$redisName->selectdb);
        $this->redis->setOption(\Redis::OPT_PREFIX, $DbConfig->$redisName->prefix);
    }
