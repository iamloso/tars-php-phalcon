...
class BaseModel extends \Phalcon\Mvc\Model
{
    ...
    {
        if (isset($this->dbService) && !empty($this->dbService)) {
            $this->setConnectionService($this->dbService);
        } else {
            $this->dbService = 'database';
            $this->setWriteConnectionService('database');
            $this->setReadConnectionService('databaseRead');
        }
    ...
    public function delete()
    {
        ...
        if (isset($this->is_delete)) {//逻辑删除
            ...
            if ($this->save() == false) {//保存状态
                ...
                $this->serviceLog('行号:'.__LINE__.' 删除数据操作： 删除数据失败， 失败信息:'.$message, 'ERROR');
                return false;
            }
            return true;
        } else {//永久删除
            if (parent::delete() == false) {//物理删除
                ...
                $this->serviceLog('行号:'.__LINE__.' 永久删除数据操作： 永久删除数据失败， 失败信息:'.$message, 'ERROR');
                return false;
            }
            return true;
        }
